...
use utils::{GAS_FOR_BASIC_OP, NO_DEPOSIT};
...
use crate::errors::*;
use crate::pool::Pool;
use crate::rated_swap::{
    rate::{global_get_rate, global_set_rate, RateTrait},
    RatedSwapPool,
};
...
pub use crate::views::{
    ContractMetadata, PoolInfo, RatedPoolInfo, RatedTokenInfo,
};
...
impl Contract {
    ...
    pub fn execute_actions(
        ...
        let result = self.internal_execute_actions(
            &mut account,
            &referral_id,
            &actions,
            ActionResult::None,
    ...
    pub fn remove_liquidity(
        ...
        if prev_storage > env::storage_usage() {
            deposits.near_amount += (prev_storage - env::storage_usage())
                as Balance
    ...
    pub fn remove_liquidity_by_tokens(
        ...
        let burn_shares = pool.remove_liquidity_by_tokens(
            &sender_id,
            amounts.clone().into_iter().map(|amount| amount.into()).collect(),
    ...
    pub fn update_token_rate(
        ...
        if let Some(rate) = global_get_rate(&token_id) {
            log!(
                "Caller {} invokes token {} rait async-update.",
                caller,
                token_id
            );
            rate.async_update()
                .then(ext_self::update_token_rate_callback(
                    token_id,
...
impl Contract {
    ...
    fn assert_no_frozen_tokens(&self, tokens: &[AccountId]) {
        let frozens: Vec<&String> = tokens
            .iter()
            .filter(|token| self.frozen_tokens.contains(*token))
            .collect();
    ...
    fn internal_execute_actions(
        ...
        for action in actions {
            result = self.internal_execute_action(
                account,
                referral_id,
                action,
                result,
...
mod tests {
    ...
    fn create_pool_with_liquidity(
        ...
        let tokens =
            token_amounts.iter().map(|(x, _)| x.clone()).collect::<Vec<_>>();
        testing_env!(context
    ...
    fn test_basics() {
        ...
        let expected_out =
            contract.get_return(0, accounts(1), one_near.into(), accounts(2));
        ...
        testing_env!(context
            ...
            .build());
        let amount_out =
            swap(&mut contract, 0, accounts(1), one_near, accounts(2));
    ...
    fn test_shares_total_supply_overflow() {
        ...
        contract.add_liquidity(
            id0,
            vec![U128(4801823983302), U128(14399)],
            None,
        );
        testing_env!(context.attached_deposit(to_yocto("0.0007")).build());
        contract.add_liquidity(
            id0,
            vec![
                U128(340282366920167 * 4801823983302),
                U128(340282366920167 * 14399),
            ],
            None,
        );
        contract.swap(
    ...
    fn test_deny_send_malicious_token() {
        ...
        testing_env!(context
            .predecessor_account_id(
                ValidAccountId::try_from("malicious").unwrap()
            )
            .build());
    ...
    fn test_send_user_specific_token() {
        ...
        testing_env!(context
            .predecessor_account_id(custom_token.clone())
            .build());
    ...
    #[test]
    #[should_panic(
        expected = "E27: attach 1yN to swap tokens not in whitelist"
    )]
    ...
    fn test_instant_swap() {
        ...
        // Get price from pool :0 1 -> 2 tokens.
        let expected_out = contract.get_return(
            0,
            accounts(1),
            to_yocto("1").into(),
            accounts(2),
    ...
    fn test_stable() {
        let (mut context, mut contract) = setup_contract();
        let token_amounts =
            vec![(accounts(1), to_yocto("5")), (accounts(2), to_yocto("5"))];
        let tokens =
            token_amounts.iter().map(|(x, _)| x.clone()).collect::<Vec<_>>();
        testing_env!(context
            .predecessor_account_id(accounts(0))
        ...
        testing_env!(context
            ...
            .build());
        let pool_id =
            contract.add_stable_swap_pool(tokens, vec![18, 18], 25, 240);
        ...
        assert_eq!(
            to_yocto("0.03"),
            contract.get_user_storage_state(accounts(3)).unwrap().deposit.0
        );
        deposit_tokens(
            &mut context,
            &mut contract,
        ...
        assert_eq!(
            to_yocto("1") + 996947470156575219215719,
            contract
                .get_deposits(accounts(3))
        ...
        let predict =
            contract.predict_remove_liquidity(pool_id, to_yocto("0.1").into());
        testing_env!(context
            .predecessor_account_id(accounts(3))
            .attached_deposit(1)
            .build());
        ...
        testing_env!(context
            ...
            .attached_deposit(1)
            .build());
        contract.stable_swap_ramp_amp(
            0,
            250,
            (3 * 86400 * 1_000_000_000).into(),
        );
        testing_env!(context
            .predecessor_account_id(accounts(0))
            .attached_deposit(1)
    ...
    fn test_rated() {
        let (mut context, mut contract) = setup_contract();
        let token_amounts =
            vec![(accounts(1), to_yocto("3")), (accounts(2), to_yocto("5"))];
        let tokens =
            token_amounts.iter().map(|(x, _)| x.clone()).collect::<Vec<_>>();
        testing_env!(context
            .predecessor_account_id(accounts(0))
        ...
        testing_env!(context
            ...
            .build());
        let pool_id =
            contract.add_rated_swap_pool(tokens, vec![18, 18], 25, 240);
        println!("{:?}", contract.version());
        ...
        let cross_call_result =
            near_sdk::serde_json::to_vec(&U128(2_000000000000000000000000))
                .unwrap();
        ...
        assert_eq!(
            pool_info.rates,
            vec![
                U128(2_000000000000000000000000),
                U128(1_000000000000000000000000)
            ]
        );

        let predict = contract.predict_add_rated_liquidity(
            pool_id,
            &vec![to_yocto("2").into(), to_yocto("4").into()],
            &Some(pool_info.rates.clone()),
        );
        testing_env!(context
            .predecessor_account_id(accounts(3))
            .attached_deposit(to_yocto("0.0007"))
        ...
        let expected_out = contract.get_rated_return(
            0,
            accounts(1),
            to_yocto("1").into(),
            accounts(2),
        ...
        assert_eq!(
            to_yocto("1") + 1992244454139326876254354,
            contract
                .get_deposits(accounts(3))
                .get(&accounts(2).to_string())
                .unwrap()
        ...
        let predict = contract.predict_remove_rated_liquidity_by_tokens(
            pool_id,
            &vec![to_yocto("0.1").into(), to_yocto("0.1").into()],
            &Some(pool_info.rates),
        );
        testing_env!(context
            .predecessor_account_id(accounts(3))
            .attached_deposit(1)
