...
use crate::errors::*;
use crate::utils::{
    add_to_collection, integer_sqrt, SwapVolume, FEE_DIVISOR,
    INIT_SHARES_SUPPLY, U256,
};
...
impl Pool {
    ...
    pub fn add_liquidity(
        ...
        assert_eq!(amounts.len(), NUM_TOKENS, "{}", ERR89_WRONG_AMOUNT_COUNT);
    ...
    pub fn swap(
        ...
        let prev_invariant = integer_sqrt(
            U256::from(self.amounts[in_idx])
                * U256::from(self.amounts[out_idx]),
        );
        ...
        let new_invariant = integer_sqrt(
            U256::from(self.amounts[in_idx])
                * U256::from(self.amounts[out_idx]),
        );
        ...
        let numerator = (new_invariant - prev_invariant)
            * U256::from(self.shares_total_supply);
        ...
        if admin_fee.exchange_fee > 0 && numerator > U256::zero() {
            let denominator =
                new_invariant * FEE_DIVISOR / admin_fee.exchange_fee;
            self.mint_shares(
                &admin_fee.exchange_id,
        ...
        if let Some(referral_id) = &admin_fee.referral_id {
            ...
            {
                let denominator =
                    new_invariant * FEE_DIVISOR / admin_fee.referral_fee;
                self.mint_shares(
                    referral_id,
                    (numerator / denominator).as_u128(),
                );
    ...
    fn internal_get_return(
        ...
        let amount_with_fee =
            U256::from(amount_in) * U256::from(FEE_DIVISOR - self.total_fee);
...
mod tests {
    ...
    fn test_pool_swap() {
        ...
        let mut pool = Pool::new(0, vec![accounts(1), accounts(2)], 30, 0, 0);
    ...
    fn test_pool_swap_with_fees() {
        ...
        testing_env!(context.build());
        let mut pool =
            Pool::new(0, vec![accounts(1), accounts(2)], 100, 100, 0);
        let mut amounts = vec![to_yocto("5"), to_yocto("10")];
        let num_shares = pool.add_liquidity(accounts(0).as_ref(), &mut amounts);
        assert_eq!(amounts, vec![to_yocto("5"), to_yocto("10")]);
        assert_eq!(
        ...
        let liq2 = pool.remove_liquidity(
            accounts(3).as_ref(),
            num_shares2,
            vec![1, 1],
        );
