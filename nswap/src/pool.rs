...
impl Pool {
    ...
    pub fn remove_liquidity_by_tokens(
        ...
        match self {
            Pool::SimplePool(_) => unimplemented!(),
            Pool::StableSwapPool(pool) => pool.remove_liquidity_by_tokens(
                sender_id,
                amounts,
                max_burn_shares,
                &admin_fee,
            ),
            Pool::RatedSwapPool(pool) => pool.remove_liquidity_by_tokens(
                sender_id,
                amounts,
                max_burn_shares,
                &admin_fee,
    ...
    pub fn get_return(
        ...
        match self {
            Pool::SimplePool(pool) => {
                pool.get_return(token_in, amount_in, token_out)
            }
            Pool::StableSwapPool(pool) => {
                pool.get_return(token_in, amount_in, token_out, fees)
            }
            Pool::RatedSwapPool(pool) => {
                pool.get_return(token_in, amount_in, token_out, fees)
            }
    ...
    pub fn swap(
        ...
        match self {
            ...
            Pool::RatedSwapPool(pool) => pool.swap(
                token_in,
                amount_in,
                token_out,
                min_amount_out,
    ...
    pub fn predict_add_stable_liquidity(
        ...
        match self {
            Pool::SimplePool(_) => unimplemented!(),
            Pool::StableSwapPool(pool) => {
                pool.predict_add_stable_liquidity(amounts, fees)
            }
            Pool::RatedSwapPool(pool) => {
                pool.predict_add_rated_liquidity(amounts, &None, fees)
            }
        }
    }
    ...
    pub fn predict_remove_liquidity_by_tokens(
        ...
        match self {
            Pool::SimplePool(_) => unimplemented!(),
            Pool::StableSwapPool(pool) => {
                pool.predict_remove_liquidity_by_tokens(amounts, fees)
            }
            Pool::RatedSwapPool(pool) => {
                pool.predict_remove_liquidity_by_tokens(amounts, fees)
            }
    ...
    pub fn predict_add_rated_liquidity(
        ...
        match self {
            ...
            Pool::StableSwapPool(_) => unimplemented!(),
            Pool::RatedSwapPool(pool) => {
                pool.predict_add_rated_liquidity(amounts, rates, fees)
            }
        }
    }
    ...
    pub fn predict_remove_rated_liquidity_by_tokens(
        ...
        match self {
            ...
            Pool::RatedSwapPool(pool) => pool
                .predict_remove_rated_liquidity_by_tokens(amounts, rates, fees),
        }
    }
    ...
    pub fn get_rated_return(
        ...
        match self {
            ...
            Pool::StableSwapPool(_) => unimplemented!(),
            Pool::RatedSwapPool(pool) => pool
                .get_rated_return(token_in, amount_in, token_out, rates, fees),
        }
    }
}
