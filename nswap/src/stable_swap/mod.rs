...
use crate::stable_swap::math::{
    Fees, StableSwap, SwapResult, MAX_AMP, MAX_AMP_CHANGE, MIN_AMP,
    MIN_RAMP_DURATION,
};
...
impl StableSwapPool {
        ...
        for (index, value) in self.token_decimals.iter().enumerate() {
            if *value <= TARGET_DECIMAL {
                let factor = 10_u128
                    ...
                    .unwrap();
                c_amounts[index] =
                    c_amounts[index].checked_mul(factor).unwrap();
            } else {
                let factor = 10_u128
                    .checked_pow((value - TARGET_DECIMAL) as u32)
                    .unwrap();
                c_amounts[index] =
                    c_amounts[index].checked_div(factor).unwrap();
            }
        }
        c_amounts
    }
    ...
    fn amount_to_c_amount(&self, amount: u128, index: usize) -> u128 {
        ...
        if *value <= TARGET_DECIMAL {
            let factor =
                10_u128.checked_pow((TARGET_DECIMAL - value) as u32).unwrap();
            amount.checked_mul(factor).unwrap()
        } else {
            let factor =
                10_u128.checked_pow((value - TARGET_DECIMAL) as u32).unwrap();
        ...
        if fee_part > 0 {
            ...
            if let Some(referral) = &fees.referral_id {
                if self.shares.get(referral).is_some() {
                    let referral_share = fee_part * fees.referral_fee as u128
                        / FEE_DIVISOR as u128;
            ...
            // exchange fee
            let exchange_share =
                fee_part * fees.exchange_fee as u128 / FEE_DIVISOR as u128;
    ...
    pub fn remove_liquidity_by_shares(
        ...
        assert_eq!(
            ...
            n_coins,
            "{}",
            ERR64_TOKENS_COUNT_ILLEGAL
        );
        let prev_shares_amount =
            self.shares.get(&sender_id).expect(ERR13_LP_NOT_REGISTERED);
        assert!(
        ...
        for i in 0..n_coins {
            result[i] = U256::from(self.c_amounts[i])
                ...
                .as_u128();
            self.c_amounts[i] =
                self.c_amounts[i].checked_sub(result[i]).unwrap();
    ...
    pub fn predict_remove_liquidity_by_tokens(
        ...
        let c_amounts = self.amounts_to_c_amounts(amounts);
        for i in 0..n_coins {
            self.assert_min_reserve(
                self.c_amounts[i].checked_sub(c_amounts[i]).unwrap_or(0),
            );
        }
    ...
    pub fn remove_liquidity_by_tokens(
        ...
        for i in 0..n_coins {
            self.c_amounts[i] =
                self.c_amounts[i].checked_sub(c_amounts[i]).unwrap();
            self.assert_min_reserve(self.c_amounts[i]);
        }
    ...
    pub fn swap(
        ...
        let result =
            self.internal_get_return(in_idx, amount_in, out_idx, &fees);
        assert!(
        ...
        if fees.referral_fee + fees.exchange_fee > 0 {
            ...
            if let Some(referral) = &fees.referral_id {
                if self.shares.get(referral).is_some() {
                    ...
                    if fee_token > 0 {
                        let referral_share = self.admin_fee_to_liquidity(
                            referral, out_idx, fee_token,
                        );
                        env::log(
                            format!(
            ...
            if fee_token > 0 {
                let exchange_share = self.admin_fee_to_liquidity(
                    &fees.exchange_id,
                    out_idx,
                    fee_token,
                );
    ...
    pub fn share_transfer(
        ...
        let balance_out =
            self.shares.get(&receiver_id).expect(ERR13_LP_NOT_REGISTERED);
    ...
    pub fn ramp_amplification(
        &mut self,
        future_amp_factor: u128,
        future_amp_time: Timestamp,
    ) {
        ...
        let amp_factor =
            invariant.compute_amp_factor().expect(ERR66_INVARIANT_CALC_ERR);
        assert!(
        ...
        assert!(
            (future_amp_factor >= amp_factor
                && future_amp_factor <= amp_factor * MAX_AMP_CHANGE)
                || (future_amp_factor < amp_factor
    ...
    pub fn stop_ramp_amplification(&mut self) {
        ...
        let amp_factor =
            invariant.compute_amp_factor().expect(ERR65_INIT_TOKEN_BALANCE);
        self.init_amp_factor = amp_factor;
...
mod tests {
    ...
    fn test_stable_julia_01() {
        ...
        let _ =
            pool.add_liquidity(accounts(0).as_ref(), &mut amounts, 1, &fees);

        let out = swap(&mut pool, 1, 10000000000, 2);
        assert_eq!(out, 9999495232);
        assert_eq!(
            pool.c_amounts,
            vec![110000000000000000000000, 90000504767247802010005]
        );
    }
    ...
    fn test_stable_romeo_01() {
        ...
        let mut amounts =
            vec![100000000000000000000000, 100000000000000000000000000000];
        let _ =
            pool.add_liquidity(accounts(0).as_ref(), &mut amounts, 1, &fees);
    ...
    fn test_stable_julia_03() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![100000000001000000000000, 99999999999000000000001]
        );
    }
    ...
    fn test_stable_romeo_03() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![100000000000000000000001, 100000000000000000000000]
        );
    }
    ...
    fn test_stable_julia_04() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![200000000000000000000000, 1556336460086846919344]
        );
    }
    ...
    fn test_stable_julia_05() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![199999000000000000000000, 1556832586795864493704]
        );
    }
    ...
    fn test_min_reserve_by_tokens() {
        ...
        pool.remove_liquidity_by_tokens(
            accounts(0).as_ref(),
            vec![amounts[0] - MIN_RESERVE, amounts[1] - MIN_RESERVE],
            shares,
            &AdminFees::zero(),
        );
        assert_eq!(vec![MIN_RESERVE, MIN_RESERVE], pool.c_amounts);
    ...
    fn test_stable_basics() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![6000000000000000000, 8999968751649207661]
        );
        let out2 = swap(&mut pool, 2, out, 1);
        assert_eq!(out2, 999999); // due to precision difference.
        assert_eq!(
            pool.c_amounts,
            vec![5000000248340316023, 9999999751649207661]
        );

        // Add only one side of the capital.
        let mut amounts2 = vec![5000000, 0];
        let num_shares =
            pool.add_liquidity(accounts(0).as_ref(), &mut amounts2, 1, &fees);
    ...
    #[test]
    #[should_panic(
        expected = "E69: pool reserved token balance less than MIN_RESERVE"
    )]
