...
pub const INIT_ACCOUNT_STORAGE: StorageUsage = ACC_ID_AS_CLT_KEY_STORAGE
    + 1
    + U128_STORAGE
    + U32_STORAGE
    + U32_STORAGE
...
impl Contract {
    ...
    pub(crate) fn internal_lostfound(
        ...
        if self.whitelisted_tokens.contains(token_id) {
            let mut lostfound =
                self.internal_unwrap_or_default_account(&self.owner_id);
            lostfound.deposit(token_id, amount);
            self.accounts.insert(&self.owner_id, &lostfound.into());
        } else {
            log!(
                "Deposit {} {} failed due to non global whitelisted token",
                amount,
                token_id
            );
        }
    ...
    pub(crate) fn internal_storage_withdraw(
        ...
        assert!(
            withdraw_amount <= available,
            "{}",
            ERR16_STORAGE_WITHDRAW_TOO_MUCH
        );
