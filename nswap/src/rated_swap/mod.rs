...
use crate::rated_swap::math::{
    Fees, RatedSwap, SwapResult, MAX_AMP, MAX_AMP_CHANGE, MIN_AMP,
    MIN_RAMP_DURATION,
};
...
impl RatedSwapPool {
    ...
    fn assert_rates_valid(&self) {
        for token_id in &self.token_account_ids {
            assert!(
                is_global_rate_valid(token_id) == true,
                "{}",
    ...
    pub fn get_amp(&self) -> u64 {
        if let Some(amp) = self
            .get_invariant_with_rates(&self.get_rates())
            .compute_amp_factor()
        {
            amp as u64
        } else {
    ...
    pub fn add_liquidity(
        ...
        let (new_shares, fee_part) = self.calc_add_liquidity_with_rates(
            amounts,
            &self.get_rates(),
            fees,
        );
    ...
    pub fn predict_remove_rated_liquidity_by_tokens(
        ...
        let invariant = self.get_invariant_with_rates(
            rates.as_ref().unwrap_or(&self.get_rates()),
        );
        let trade_fee = Fees::new(self.total_fee, &fees);
    ...
    fn internal_get_return(
        ...
        self.assert_rates_valid();
        self.internal_get_return_with_rates(
            token_in,
            amount_in,
            token_out,
            &self.get_rates(),
    ...
    pub fn get_rated_return(
        ...
        let c_amount_out = self
            .internal_get_return_with_rates(
    ...
    pub fn swap(
        ...
        assert!(amount_swapped >= min_amount_out, "{}", ERR68_SLIPPAGE);
        env::log(
            format!(
...
mod tests {
    ...
    fn test_rated_julia_07() {
        ...
        assert_eq!(
            pool.c_amounts,
            vec![100001 * PRECISION, 199998_000000009995002449799089]
        );
    }
