...
pub static RATES: Lazy<Mutex<HashMap<AccountId, Rate>>> =
...
pub fn global_register_rate(rate_type: &String, token_id: &AccountId) -> bool {
    assert!(
        Rate::is_valid_rate_type(rate_type.as_str()),
        "{}",
        ERR127_INVALID_RATE_TYPE
    );
    // read from storage
    let mut rates: HashMap<String, Rate> =
    ...
    if !rates.contains_key(token_id) {
        rates.insert(
            token_id.clone(),
            Rate::new(rate_type.clone(), token_id.clone()),
        );
...
pub fn global_get_rate(token_id: &AccountId) -> Option<Rate> {
    if RATES.lock().unwrap().is_empty() {
        let rates: HashMap<AccountId, Rate> = if let Some(content) =
