...
impl RatedSwap {
    ...
    pub fn compute_amp_factor(&self) -> Option<Balance> {
        if self.current_ts < self.stop_ramp_ts {
            let time_range =
                self.stop_ramp_ts.checked_sub(self.start_ramp_ts)?;
    ...
    pub fn compute_d(&self, c_amounts: &Vec<Balance>) -> Option<U384> {
        ...
        if sum_x == 0 {
            ...
            for _ in 0..256 {
                ...
                let ann = amp_factor
                    .checked_mul(n_coins.checked_pow(n_coins as u32)?.into())?;
    ...
    pub fn compute_y(
        ...
        c = c.checked_mul(d)?.checked_div(
            ann.checked_mul(
                (n_coins as u128).checked_pow(n_coins as u32)?.into(),
            )?
            .into(),
        )?;
        ...
        for _ in 0..256 {
            ...
            let y_denominator =
                y.checked_mul(2.into())?.checked_add(b)?.checked_sub(d)?;
    ...
    pub fn swap_to(
        ...
        let dy = current_c_amounts_rated[token_out_idx]
            .checked_sub(y)?
            .checked_sub(1)
            .unwrap_or(0_u128); // * ? curve sub -1 just in case there were some rounding errors
