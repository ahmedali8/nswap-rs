...
use crate::utils::{
    GAS_FOR_FT_TRANSFER_CALL, GAS_FOR_RESOLVE_TRANSFER, NO_DEPOSIT,
};
use crate::*;
...
impl Contract {
        ...
        match parse_token_id(token_id) {
            ...
            TokenOrPool::Token(token_id) => {
                ...
                let mut receiver_account: Account =
                    self.internal_unwrap_account(&receiver_id);
    ...
    pub fn mft_balance_of(
        ...
        self.internal_mft_balance(token_id, account_id.as_ref()).into()
    }
    ...
    pub fn mft_resolve_transfer(
        ...
        if unused_amount > 0 {
            let receiver_balance =
                self.internal_mft_balance(token_id.clone(), &receiver_id);
            if receiver_balance > 0 {
                let refund_amount =
                    std::cmp::min(receiver_balance, unused_amount);
