...
#[test]
fn sim_stable_swap() {
    let (root, _owner, pool, tokens) = setup_stable_pool_with_liquidity(
        vec![dai(), usdt(), usdc()],
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amp: 10000,
            token_account_ids: tokens
                .into_iter()
                .map(|x| x.account_id())
                .collect(),
            amounts: vec![
                U128(100000 * ONE_DAI),
                U128(100000 * ONE_USDT),
                U128(100000 * ONE_USDC)
            ],
            total_fee: 25,
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100002 * ONE_DAI),
                U128(99999 * ONE_USDT + 2500),
                U128(99999 * ONE_USDC + 2500)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                300000 * ONE_LPT + 499999996666583 + 499999993277742
            ),
        }
    );
}
...
fn sim_stable_lp() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100500 * ONE_DAI),
                U128(100600 * ONE_USDT),
                U128(100800 * ONE_USDC)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                301200 * ONE_LPT + 699699997426210330025 + 47999999735823255
            ),
        }
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        last_lpt_supply + 699699997426210330025 + 47999999735823255
    );
    let last_lpt_supply =
        last_lpt_supply + 699699997426210330025 + 47999999735823255;
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity_by_tokens(
            0,
            vec![U128(1 * ONE_DAI), U128(500 * ONE_USDT), U128(1 * ONE_USDC)],
            U128(550 * ONE_LPT)
        ),
        deposit = 1
    );
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100499 * ONE_DAI),
                U128(100100 * ONE_USDT),
                U128(100799 * ONE_USDC)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                last_lpt_supply - 502598491280079770545 + 95823884420348155
            ),
        }
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user1.account_id()),
        1200 * ONE_LPT - 502598491280079770545
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        699699997426210330025
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user1.account_id()),
        1100 * ONE_LPT - 502598491280079770545
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        799699997426210330025
    );
    assert_eq!(pool_share_price(&pool, 0), last_share_price);
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(300 * ONE_LPT),
            vec![U128(1 * ONE_DAI), U128(298 * ONE_USDT), U128(1 * ONE_USDC)]
        ),
        deposit = 1
    );
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity_by_tokens(
            0,
            vec![U128(498 * ONE_DAI), U128(0 * ONE_USDT), U128(0 * ONE_USDC)],
            U128(499 * ONE_LPT)
        ),
        deposit = 1
    );
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        499699997426210330025 - 498596260777261245961
    );
    // last_lpt_supply - removed lpt + admin_fee_to_lpt
    let last_lpt_supply =
        last_lpt_supply - 498596260777261245961 + 95600058313712936;
    assert_eq!(mft_total_supply(&pool, ":0"), last_lpt_supply);
    assert!(pool_share_price(&pool, 0) > last_share_price);
    let last_share_price = pool_share_price(&pool, 0);
    ...
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![
                U128(100_000_000_000 * ONE_DAI),
                U128(100_000_000_000 * ONE_USDT),
                U128(100_000_000_000 * ONE_USDC)
            ],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
...
fn sim_stable_lp_storage() {
    let (root, _owner, pool, tokens) = setup_stable_pool_with_liquidity(
        vec![
            "dai1234567890123456789012345678901234567890123456789012345678901"
                .to_string(),
            usdt(),
            "cusd123456789012345678901234567890123456789012345678901234567890"
                .to_string(),
        ],
    ...
    let user = root.create_user("user".to_string(), to_yocto("100"));
    call!(user, usdt_token.mint(user.valid_account_id(), U128(500 * ONE_USDT)))
        .assert_success();
    ...
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![U128(0), U128(500 * ONE_USDT), U128(0)],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
    );
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(300 * ONE_LPT),
            vec![U128(1 * ONE_DAI), U128(1 * ONE_USDT), U128(1 * ONE_CUSD)]
        ),
        deposit = 1
    );
    assert!(!out_come.is_ok());
