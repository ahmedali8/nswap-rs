...
fn frozenlist_scenario_01() {
    ...
    println!(
        "Frozenlist Case 0101: only owner and guardians can manage frozenlist"
    );

    let out_come = call!(
        guard1,
        pool.extend_frozenlist_tokens(vec![
            to_va(eth()),
            to_va(dai()),
            to_va(usdt())
        ]),
...
fn frozenlist_scenario_02() {
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(to_yocto("1")),
            vec![U128(to_yocto("1")), U128(to_yocto("2"))]
        ),
        deposit = 1
    );
    assert!(!out_come.is_ok());
...
fn frozenlist_scenario_03() {
    let (root, owner, pool, _tokens) = setup_stable_pool_with_liquidity(
        vec![dai(), usdt(), usdc()],
