...
use near_sdk_sim::{
    call, init_simulator, runtime, to_yocto, view, ExecutionResult,
};
...
fn assert_failure(outcome: ExecutionResult, error_message: &str) {
    ...
    let exe_status =
        format!("{:?}", outcome.promise_errors()[0].as_ref().unwrap().status());
    println!("{}", exe_status);
...
fn sim_stable_e61() {
    ...
    call!(
        owner,
        ex.extend_whitelisted_tokens(vec![
...
fn sim_stable_e63() {
    ...
    call!(
        ...
        ex.add_stable_liquidity(
            0,
            vec![U128(1 * ONE_DAI), U128(1 * ONE_USDT)],
            U128(1)
        ),
...
fn sim_stable_e65() {
    ...
    let outcome = call!(
        ...
        ex.add_stable_liquidity(
            0,
            vec![U128(1 * ONE_DAI), U128(0 * ONE_USDT)],
            U128(1)
        ),
        deposit = to_yocto("0.01")
    );
...
fn sim_stable_e34() {
    ...
    let lp_shares =
        view!(ex.mft_balance_of(":0".to_string(), root.valid_account_id()))
            .unwrap_json::<U128>();
...
fn sim_stable_e82() {
    ...
    let outcome =
        call!(owner, ex.stable_swap_ramp_amp(0, 0, U64(0)), deposit = 1);
