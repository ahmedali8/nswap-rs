...
use near_sdk_sim::{
    call, deploy, init_simulator, to_yocto, view, ContractAccount,
    ExecutionResult, UserAccount,
};
use nswap::{ContractContract as Exchange, PoolInfo};
use rand::Rng;
use rand_pcg::Pcg32;
...
near_sdk_sim::lazy_static_include::lazy_static_include_bytes! {
    TEST_TOKEN_WASM_BYTES => "../build/test_token.wasm",
    EXCHANGE_WASM_BYTES => "../build/nswap.wasm",
}
...
pub fn test_token(
    ...
    call!(
        ...
        t.mint(
            ...
            to_yocto(&format!("{}", INIT_ACCOUNT_FOR_TOKEN)).into()
        )
    )
    .assert_success();
...
pub fn get_token_amount_in_pool(
    simple_pool_info: &PoolInfo,
    token_account_id: &AccountId,
) -> u128 {
    simple_pool_info.amounts[simple_pool_info
        .token_account_ids
        .iter()
        .position(|id| id == token_account_id)
        .unwrap()]
    .0
}

pub fn user_storage_deposit(
...
pub fn user_init_deposit_token(
    ...
    call!(
        &operator.user,
        token_contract2.ft_transfer_call(
            to_va(swap()),
            to_yocto(&init_token.to_string()).into(),
            None,
            "".to_string()
        ),
        deposit = 1
...
pub fn user_init_token_account(
    ctx: &mut OperationContext,
    root: &UserAccount,
    operator: &Operator,
    token: &AccountId,
...
pub fn add_token_deposit(
    ...
    if token_amount < need_value - current_value {
        println!(
            "mint {} {} to {}",
            INIT_ACCOUNT_FOR_TOKEN,
            token_contract.account_id(),
...
pub fn current_evn_info(
    ctx: &mut OperationContext,
    pool: &ContractAccount<Exchange>,
    operator: &Operator,
    tokens: &Vec<String>,
) -> (Scenario, u128, u128, u128, u128) {
    let storage_state =
        view!(pool.get_user_storage_state(operator.user.valid_account_id()))
            .unwrap_json_value();
    ...
    let token_contract1 =
        ctx.token_contract_account.get(tokens.get(0).unwrap()).unwrap();
    let token_contract2 =
        ctx.token_contract_account.get(tokens.get(1).unwrap()).unwrap();
    let token1_account =
        view!(token_contract1
            .ft_balance_of(to_va(operator.user.account_id.clone())))
        .unwrap_json::<U128>()
        .0;
    println!(
        "{} has {} balance : {}",
        operator.user.account_id,
        token_contract1.account_id(),
        token1_account
    );
    if token1_account == 0 {
    ...
    let pool_deposits =
        view!(pool.get_deposits(to_va(operator.user.account_id.clone())))
...
pub fn init_pool_env(
) -> (UserAccount, UserAccount, ContractAccount<Exchange>, Vec<Operator>) {
    ...
    call!(root, pool.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();

    call!(owner, pool.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();
    ...
    for user_id in 0..EVERY_PREFERENCE_NUM {
        let user = Operator {
            user: root.create_user(
                format!("user_create_sample_pool_{}", user_id),
                to_yocto("100"),
            ),
        ...
        let user = Operator {
            user: root.create_user(
                format!("user_direct_swap_{}", user_id),
                to_yocto("100"),
            ),
        ...
        let user = Operator {
            user: root.create_user(
                format!("user_pool_swap_{}", user_id),
                to_yocto("100"),
            ),
        ...
        let user = Operator {
            user: root.create_user(
                format!("user_add_liquidity_{}", user_id),
                to_yocto("100"),
            ),
    ...
    call!(
        ...
        pool.extend_whitelisted_tokens(
            TOKENS.map(|v| to_va(v.to_string())).to_vec()
        ),
        deposit = 1
    );
...
pub fn setup_stable_pool_with_liquidity_and_operators(
    ...
    let mut users = Vec::new();
    for user_id in 0..EVERY_PREFERENCE_NUM {
        let user = root.create_user(
            format!("user_remove_stable_liquidity_by_share_{}", user_id),
            to_yocto("100"),
        );
        ...
        let user = root.create_user(
            format!("user_remove_stable_liquidity_by_token_{}", user_id),
            to_yocto("100"),
        );
        call!(user, pool.storage_deposit(None, None), deposit = to_yocto("1"))
            .assert_success();
        ...
        let user = root.create_user(
            format!("user_pool_stable_swap_{}", user_id),
            to_yocto("100"),
        );
        call!(user, pool.storage_deposit(None, None), deposit = to_yocto("1"))
            .assert_success();
        ...
        let user = root.create_user(
            format!("user_add_stable_liquidity_{}", user_id),
            to_yocto("100"),
        );
        call!(user, pool.storage_deposit(None, None), deposit = to_yocto("1"))
            .assert_success();
...
pub fn add_and_deposit_token(
    ...
    if 0 == view!(token.ft_balance_of(user.valid_account_id()))
        .unwrap_json::<U128>()
        .0
    {
        call!(user, token.mint(user.valid_account_id(), U128(10)))
            .assert_success();
    }
    ...
    call!(user, ex.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();
...
pub fn mft_balance_of(
    ...
    view!(pool
        .mft_balance_of(token_or_pool.to_string(), to_va(account_id.clone())))
    .unwrap_json::<U128>()
    .0
}
...
pub fn pool_share_price(
    ...
    view!(pool.get_pool_share_price(pool_id)).unwrap_json::<U128>().0
}
