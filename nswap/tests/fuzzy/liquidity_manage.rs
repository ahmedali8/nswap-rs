#![allow(unused)]
use crate::fuzzy::{constants::*, types::*, utils::*};
...
pub fn add_liquidity_action(
    ...
    call!(
        ...
        pool.add_liquidity(
            simple_pool_id,
            vec![U128(liquidity1), U128(liquidity2)],
            None
        ),
            ...
            fair_supply = min(
                ...
                U256::from(amounts[i])
                    * U256::from(simple_pool_info.shares_total_supply.0)
                    / simple_pool_info.amounts[i].0,
            );
        }
        for i in 0..simple_pool_info.token_account_ids.len() {
            let amount = (U256::from(simple_pool_info.amounts[i].0)
                * fair_supply
                / U256::from(simple_pool_info.shares_total_supply.0))
            .as_u128();
            if i == 0 {
...
pub fn do_add_liquidity(
    ...
    let (liquidity1, liquidity2) = (
        to_yocto(&ADD_LIQUIDITY_LIMIT.to_string()),
        to_yocto(&ADD_LIQUIDITY_LIMIT.to_string()),
    );
    ...
    loop {
        ...
        match scenario {
            Scenario::Normal => {
                let (real_liquidity1, real_liquidity2) = match real_liquidity(
                    pool,
                    simple_pool_id,
                    vec![liquidity1, liquidity2],
                ) {
                    Some((real_liquidity1, real_liquidity2)) => {
                        (real_liquidity1, real_liquidity2)
                    }
                    None => (liquidity1, liquidity2),
                };

                if token1_deposit < real_liquidity1 {
                    let out_come = add_liquidity_action(
                        pool,
                        operator,
                        simple_pool_id,
                        liquidity1,
                ...
                println!(
                    "after add liquidity current simple pool info {:?} ",
            ...
            Scenario::Token2NoAccount => {
                let (real_liquidity1, _) = match real_liquidity(
                    pool,
                    simple_pool_id,
                    vec![liquidity1, liquidity2],
                ) {
                ...
                let token1_deposit = view!(pool.get_deposit(
                    to_va(operator.user.account_id.clone()),
                    to_va(tokens.get(0).unwrap().clone())
                ))
                .unwrap_json::<U128>()
                .0;
                let out_come = add_liquidity_action(
...
pub fn do_stable_add_liquidity(
    ...
    let add_amounts = vec![
        U128(rng.gen_range(1..ADD_LIQUIDITY_LIMIT as u128) * ONE_DAI),
    ...
    let cal_share = view!(pool.predict_add_stable_liquidity(0, &add_amounts))
        .unwrap_json::<U128>()
        .0;
...
pub fn do_stable_remove_liquidity_by_shares(
    token_contracts: &Vec<ContractAccount<TestToken>>,
    rng: &mut Pcg32,
    root: &UserAccount,
    operator: &StableOperator,
    ...
    let min_amounts =
        vec![U128(1 * ONE_DAI), U128(1 * ONE_USDT), U128(1 * ONE_USDC)];
    ...
    if scenario == StableScenario::Normal {
        increase_amounts =
            view!(pool.predict_remove_liquidity(0, U128(remove_lp_num)))
                .unwrap_json::<Vec<U128>>();
    }
    ...
    println!(
        "do_stable_remove_liquidity_by_shares scenario : {:?} begin!",
        scenario
    );
    match scenario {
        StableScenario::Normal => {
            ...
            for (idx, item) in increase_amounts.iter().enumerate() {
                assert_eq!(
                    balances.get(STABLE_TOKENS[idx]).unwrap().0,
    ...
    println!(
        "do_stable_remove_liquidity_by_shares scenario : {:?} end!",
        scenario
    );
}

pub fn do_stable_remove_liquidity_by_token(
    token_contracts: &Vec<ContractAccount<TestToken>>,
    rng: &mut Pcg32,
    root: &UserAccount,
    operator: &StableOperator,
    ...
    let remove_amounts = vec![
        U128(rng.gen_range(1..REMOVE_LIQUIDITY_LIMIT as u128) * ONE_DAI),
    ...
    let remove_lpt =
        view!(pool.predict_remove_liquidity_by_tokens(0, &remove_amounts))
            .unwrap_json::<U128>()
            .0;
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity_by_tokens(
            ...
            U128(max_burn_shares)
        ),
        deposit = 1
    );

    println!(
        "do_stable_remove_liquidity_by_token scenario : {:?} begin!",
        scenario
    );
    match scenario {
        StableScenario::Normal => {
    ...
    println!(
        "do_stable_remove_liquidity_by_token scenario : {:?} end!",
        scenario
    );
}
