#![allow(unused)]
use crate::fuzzy::{constants::*, liquidity_manage::*, types::*, utils::*};
use near_sdk::json_types::U128;
use near_sdk::AccountId;
use near_sdk_sim::{
    call, to_yocto, view, ContractAccount, ExecutionResult, UserAccount,
};
use nswap::{ContractContract as Exchange, PoolInfo, SwapAction};
use rand::Rng;
use rand_pcg::Pcg32;
...
pub fn do_pool_swap(
    ...
    operator: &Operator,
    pool: &ContractAccount<Exchange>,
    simple_pool_count: u64,
) {
    let simple_pool_id = if simple_pool_count == 0 {
        0
    } else {
        rng.gen_range(0..simple_pool_count)
    };
    let simple_pool_info =
        view!(pool.get_pool(simple_pool_id)).unwrap_json::<PoolInfo>();
    ...
    loop {
        ...
        let token_in_pool_amount =
            get_token_amount_in_pool(&simple_pool_info, token_in);
        let token_out_pool_amount =
            get_token_amount_in_pool(&simple_pool_info, token_out);
        ...
        match scenario {
            Scenario::Normal => {
                if token_in_pool_amount == 0 || token_out_pool_amount == 0 {
                    ...
                    if amount_in > token_in_deposit {
                        assert!(get_error_status(&out_come)
                            .contains("E22: not enough tokens in deposit"));
                    } else {
                        assert!(get_error_status(&out_come)
                            .contains("E76: invalid params"));
                    }
                    do_add_liquidity(
                        ctx,
                        rng,
                ...
                let pool_deposits =
                    view!(pool
                        .get_deposits(to_va(operator.user.account_id.clone())))
                    .unwrap_json::<HashMap<AccountId, U128>>();
                let token_in_deposit_old =
                    pool_deposits.get(token_in).unwrap().0;
                let token_out_deposit_old =
                    pool_deposits.get(token_out).unwrap().0;
                ...
                if amount_in > token_in_deposit_old {
                    ...
                    assert!(get_error_status(&out_come)
                        .contains("E22: not enough tokens in deposit"));
                    println!(
                        "token_amount: {} need_amount: {}  current_amount: {}",
                        token_in_amount, amount_in, token_in_deposit_old
                    );
                    add_token_deposit(
                        ctx,
                        root,
                        ...
                        token_in_amount,
                        amount_in,
                        token_in_deposit_old,
                    );
                }

                let swap_amount_budget = view!(pool.get_return(
                    simple_pool_id,
                    to_va(token_in.clone()),
                    U128(amount_in),
                    to_va(token_out.clone())
                ))
                .unwrap_json::<U128>()
                .0;

                let swap_amount_string = swap_action(
                    pool,
                    operator,
                    token_in.clone(),
                    token_out.clone(),
                ...
                assert_eq!(
                    token_out_deposit,
                    swap_amount + token_out_deposit_old
                );
                assert_eq!(swap_amount, swap_amount_budget);
                let new_simple_pool_info = view!(pool.get_pool(simple_pool_id))
                    .unwrap_json::<PoolInfo>();
                println!(
                    "after pool swap current simple pool info {:?} ",
                    new_simple_pool_info
                );
                break;
            }
            Scenario::Token1NoAccount => {
                if is_shuffle % 2 == 1 {
                    let account_tokens = view!(pool
                        .get_user_whitelisted_tokens(to_va(
                            operator.user.account_id.clone()
                        )))
            ...
            Scenario::Token2NoAccount => {
                let account_tokens = view!(pool.get_user_whitelisted_tokens(
                    to_va(operator.user.account_id.clone())
                ))
                .unwrap_json::<Vec<AccountId>>();
                let out_come = swap_action(
                    pool,
                ...
                if account_tokens.contains(token_in) {
                    assert_eq!(get_error_count(&out_come), 1);
                    println!("{}", get_error_status(&out_come));
                    assert!(get_error_status(&out_come).contains(
                        "Smart contract panicked: panicked at 'ERR_INVALID'"
                    ));
                } else {
                    assert_eq!(get_error_count(&out_come), 1);
                    assert!(get_error_status(&out_come)
            ...
            Scenario::NoStorageDeposit => {
                ...
                assert_eq!(get_error_count(&out_come), 1);
                assert!(get_error_status(&out_come)
                    .contains("E10: account not registered"));
...
pub fn get_swap_info(
    rng: &mut Pcg32,
) -> (AccountId, AccountId, u128, usize, usize) {
    ...
    loop {
        ...
        return (
            STABLE_TOKENS[token_in_index].to_string(),
            STABLE_TOKENS[token_out_index].to_string(),
            amount_in_unit[token_in_index] * amount_in,
            token_in_index,
            token_out_index,
        );
    }
...
pub fn do_stable_pool_swap(
    token_contracts: &Vec<ContractAccount<TestToken>>,
    rng: &mut Pcg32,
    root: &UserAccount,
    ...
    let balances = view!(pool.get_deposits(operator.user.valid_account_id()))
        .unwrap_json::<HashMap<AccountId, U128>>();
    println!("current user balance: {:?}", balances);

    let (token_in, token_out, amount_in, token_in_index, token_out_index) =
        get_swap_info(rng);

    let token_contract = token_contracts.get(token_in_index).unwrap();

    println!("swap  {} => {} : {}", token_in, token_out, amount_in);
    add_and_deposit_token(
        root,
        &operator.user,
        token_contract,
        pool,
    ...
    let swap_out = view!(pool.get_return(
        0,
        to_va(token_in.clone()),
        U128(amount_in),
        to_va(token_out.clone())
    ...
    .0;
    if swap_out
        > view!(pool.get_pool(0)).unwrap_json::<PoolInfo>().amounts
            [token_out_index]
            .0
    {
    ...
    match scenario {
        StableScenario::Normal => {
            ...
            assert_eq!(
                view!(pool.get_deposits(operator.user.valid_account_id()))
                    .unwrap_json::<HashMap<AccountId, U128>>()
                    .get(&token_in)
                    .unwrap()
                    .0,
        ...
        StableScenario::Slippage => {
            assert_eq!(get_error_count(&out_come), 1);
            assert!(get_error_status(&out_come)
                .contains("E34: insufficient lp shares"));
            assert_eq!(
                view!(pool.get_deposits(operator.user.valid_account_id()))
                    .unwrap_json::<HashMap<AccountId, U128>>()
                    .get(&token_in)
        ...
        _ => {
            panic!(
                "do_stable_pool_swap find new StableScenario {:?}",
                scenario
            );
        }
    }
