...
pub fn do_direct_swap(
    ...
    loop {
        ...
        let test_token_in_amount =
            get_test_token_amount(ctx, operator, token_in);
        let test_token_out_amount =
            get_test_token_amount(ctx, operator, token_out);
        ...
        match scenario {
            DSScenario::Normal => {
                ...
                let out_come = direct_swap_action(
                    ctx,
                    &operator.user,
                    token_in,
                    vec![action.clone()],
                ...
                let test_token_in_amount_new =
                    get_test_token_amount(ctx, operator, token_in);
                let test_token_out_amount_new =
                    get_test_token_amount(ctx, operator, token_out);
                ...
                if swap_amount_budget < min_amount_out {
                    ...
                    assert_eq!(
                        test_token_out_amount,
                        test_token_out_amount_new
                    );
                } else {
                    assert_eq!(
                        test_token_in_amount - amount_in,
                        test_token_in_amount_new
                    );
                    assert_eq!(
                        test_token_out_amount + swap_amount_budget,
                        test_token_out_amount_new
                    );
                }
            ...
            DSScenario::TokenInZero => {
                ...
                assert_eq!(get_error_count(&out_come), 1);
                assert!(get_error_status(&out_come)
                    .contains("Smart contract panicked: The account"));
                assert!(
                    get_error_status(&out_come).contains("is not registered")
                );
