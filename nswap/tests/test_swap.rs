...
fn setup_pool_with_liquidity() -> (
    ...
    call!(
        ...
        pool.add_liquidity(
            0,
            vec![U128(to_yocto("5")), U128(to_yocto("10"))],
            None
        ),
...
fn test_withdraw_failure() {
    ...
    call!(
        root,
        weth_contract.ft_transfer_call(
            to_va(swap()),
            to_yocto("110").into(),
            None,
            "".to_string()
    ...
    let balances_before =
        view!(pool.get_deposits(to_va(root.account_id.clone())))
            .unwrap_json::<HashMap<AccountId, U128>>();
    ...
    let mut dai_amount: U128 =
        view!(dai_contract.ft_balance_of(to_va("root".to_string())))
            .unwrap_json();
    assert_eq!(dai_amount, to_yocto("895").into());

    // User (perhaps accidentally) unregisters account from fungible token.
    call!(root, dai_contract.storage_unregister(Some(true)), deposit = 1)
        .assert_success();

    // Now DAI balance for root is now 0, with no storage either
    dai_amount = view!(dai_contract.ft_balance_of(to_va("root".to_string())))
        .unwrap_json();
...
fn test_direct_swap() {
    ...
    assert_eq!(balance_of(&token2, &new_user.account_id), to_yocto("0"));
    assert!(
        mft_balance_of(&pool, &token2.account_id(), &owner.account_id)
            > to_yocto("1")
    );
    ...
    assert!(
        mft_balance_of(&pool, &token2.account_id(), &new_user.account_id)
            > to_yocto("0.5")
    );
}
