...
use nswap::{ContractContract as Exchange, ContractMetadata, PoolInfo};
use test_rated_token::ContractContract as TestRatedToken;
use test_token::ContractContract as TestToken;

near_sdk_sim::lazy_static_include::lazy_static_include_bytes! {
    TEST_TOKEN_WASM_BYTES => "../build/test_token.wasm",
    TEST_RATED_TOKEN_WASM_BYTES => "../build/test_rated_token.wasm",
...
pub fn test_rated_token(
    ...
    call!(
        root,
        t.new(token_id.clone(), token_id.clone(), 24, U128(10u128.pow(24)))
    )
    .assert_success();
    call!(
        root,
...
pub fn get_user_tokens(
    ...
    view!(pool.get_user_whitelisted_tokens(account_id))
        .unwrap_json::<Vec<String>>()
}
...
pub fn setup_pool_with_liquidity() -> (
    ...
    call!(
        owner,
        pool.extend_whitelisted_tokens(vec![
            to_va(dai()),
            to_va(eth()),
            to_va(usdt())
        ]),
    ...
    call!(
        ...
        pool.add_liquidity(
            0,
            vec![U128(to_yocto("10")), U128(to_yocto("20"))],
            None
        ),
    ...
    call!(
        ...
        pool.add_liquidity(
            1,
            vec![U128(to_yocto("20")), U128(to_yocto("10"))],
            None
        ),
        deposit = to_yocto("0.0007")
    )
    ...
    call!(
        ...
        pool.add_liquidity(
            2,
            vec![U128(to_yocto("10")), U128(to_yocto("10"))],
            None
        ),
        deposit = to_yocto("0.0007")
    )
...
pub fn setup_rated_pool_with_liquidity(
    ...
    let mut token_rated_contracts: Vec<ContractAccount<TestRatedToken>> =
        vec![];
    for token_name in &reated_tokens {
        pool_tokens.push(to_va(token_name.clone()));
        token_rated_contracts.push(test_rated_token(
            &root,
            token_name.clone(),
            vec![swap()],
        ));
    ...
    call!(
        owner,
        pool.add_rated_swap_pool(pool_tokens, decimals, pool_fee, amp),
        deposit = to_yocto("1")
    )
    .assert_success();
    ...
    assert_eq!(
        100000000,
        view!(pool.get_pool_share_price(0)).unwrap_json::<U128>().0
    );
    ...
    call!(
        ...
        pool.add_stable_liquidity(
            0,
            all_amounts.map(|x| U128(*x)).collect(),
            U128(1)
        ),
        deposit = to_yocto("0.0007")
    )
...
pub fn mint_and_deposit_rated_token(
    ...
    call!(user, ex.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();
    call!(user, token.mint(user.valid_account_id(), U128(amount)))
        .assert_success();
    call!(
        user,
...
pub fn whitelist_token(
    ...
    call!(owner, ex.extend_whitelisted_tokens(tokens), deposit = 1)
        .assert_success();
}
...
pub fn deposit_token(
    ...
    for (idx, token) in tokens.into_iter().enumerate() {
        call!(user, ex.storage_deposit(None, None), deposit = to_yocto("0.1"))
            .assert_success();
        call!(
            user,
            token.ft_transfer_call(
