...
use nswap::{RunningState, SwapAction};
pub mod common;
...
fn guardians_scenario_01() {
    ...
    let out_come = call!(
        root,
        pool.extend_guardians(vec![
            guard1.valid_account_id(),
            guard2.valid_account_id()
        ]),
        deposit = 1
    );
    ...
    assert!(get_error_status(&out_come)
        .contains("E100: no permission to invoke this"));
    ...
    let out_come = call!(
        owner,
        pool.remove_whitelisted_tokens(vec![
            to_va(usdt()),
            to_va(eth()),
            to_va(dai())
        ]),
    ...
    println!(
        "Guardians Case 0103: only owner and guardians can pause the contract"
    );
    let out_come =
        call!(root, pool.change_state(RunningState::Paused), deposit = 1);
    assert!(!out_come.is_ok());
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come)
        .contains("E100: no permission to invoke this"));
    ...
    let out_come =
        call!(guard1, pool.change_state(RunningState::Paused), deposit = 1);
    out_come.assert_success();
    assert_eq!(get_error_count(&out_come), 0);
    let metadata = get_metadata(&pool);
    ...
    let out_come =
        call!(root, pool.change_state(RunningState::Running), deposit = 1);
    assert!(!out_come.is_ok());
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come)
        .contains("E100: no permission to invoke this"));
    ...
    let out_come =
        call!(guard2, pool.change_state(RunningState::Running), deposit = 1);
    assert!(!out_come.is_ok());
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come)
        .contains("E100: no permission to invoke this"));
    ...
    let out_come =
        call!(owner, pool.change_state(RunningState::Running), deposit = 1);
    out_come.assert_success();
    assert_eq!(get_error_count(&out_come), 0);
    let metadata = get_metadata(&pool);
    assert_eq!(metadata.state, RunningState::Running);
...
fn guardians_scenario_02() {
    let (root, old_owner, pool, _, token2, token3) =
        setup_pool_with_liquidity();
    let guard1 = root.create_user("guard1".to_string(), to_yocto("100"));
    let owner = root.create_user("owner2".to_string(), to_yocto("100"));
    call!(old_owner, pool.set_owner(owner.valid_account_id()), deposit = 1)
        .assert_success();
    call!(
        owner,
    ...
    .assert_success();
    call!(owner, pool.modify_admin_fee(1600, 400), deposit = 1)
        .assert_success();
    call!(
        root,
        pool.swap(
    ...
    .assert_success();
    assert_eq!(
        mft_balance_of(&pool, ":1", &pool.account_id()),
    ...
    let out_come = call!(
        guard1,
        pool.remove_exchange_fee_liquidity(
            1,
            U128(18182851357079036914),
            vec![U128(1), U128(1)]
        ),
        deposit = 1
    );
    ...
    assert_eq!(
        owner_deposits.get(&token2.account_id()).unwrap().0,
        330666437772348207866
    );
    assert_eq!(
        owner_deposits.get(&token3.account_id()).unwrap().0,
    ...
    let out_come = call!(
        guard1,
        pool.withdraw_owner_token(
            token2.valid_account_id(),
            U128(330666437772348207866)
        ),
        deposit = 1
    );
    out_come.assert_success();
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come)
        .contains("The account owner2 is not registered"));
    let owner_deposits = get_deposits(&pool, owner.valid_account_id());
    assert_eq!(
        owner_deposits.get(&token2.account_id()).unwrap().0,
        330666437772348207866
