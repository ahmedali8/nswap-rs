...
use nswap::{PoolInfo, RatedTokenInfo, SwapAction};
pub mod common;
...
fn sim_rated_swap_liquidity_two() {
    let (root, owner, pool, tokens, token_rated_contracts) =
        setup_rated_pool(vec![near()], vec![stnear()], vec![24, 24], 25, 10000);
    ...
    call!(root, stnear_contract.set_price(U128(2 * 10u128.pow(24))))
        .assert_success();

    call!(
        owner,
        pool.update_token_rate(stnear_contract.valid_account_id()),
        deposit = 1
    )
    .assert_success();
    ...
    mint_and_deposit_rated_token(
        &user,
        &token_rated_contracts[0],
        &pool,
        100000 * ONE_STNEAR,
    );
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![U128(100000 * ONE_NEAR), U128(50000 * ONE_STNEAR)],
            U128(1)
        ),
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user.account_id()),
        200000 * ONE_LPT
    );
    ...
    println!("{:#?}", get_logs(&out_come));
    assert_eq!(
        mft_balance_of(&pool, ":0", &user1.account_id()),
        200000 * ONE_LPT
    );
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(200000 * ONE_LPT),
            vec![U128(1 * ONE_NEAR), U128(1 * ONE_STNEAR)]
        ),
        deposit = 1
    );
    out_come.assert_success();
    ...
    assert!(get_error_status(&out_come)
        .contains("E69: pool reserved token balance less than MIN_RESERVE"));
}

#[test]
fn sim_rated_swap_liquidity_three_one_rated() {
    let (root, owner, pool, tokens, token_rated_contracts) = setup_rated_pool(
        vec![near()],
        vec![stnear(), linear()],
        vec![24, 24, 24],
        25,
    ...
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![
                U128(100000 * ONE_NEAR),
                U128(50000 * ONE_STNEAR),
                U128(100000 * ONE_LINEAR)
            ],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
...
fn sim_rated_swap_liquidity_three_two_rated() {
    ...
    call!(root, linear_contract.set_price(U128(4 * 10u128.pow(24))))
        .assert_success();
    ...
    call!(
        owner,
        pool.update_token_rate(linear_contract.valid_account_id()),
        deposit = 1
    )
    .assert_success();
...
fn sim_rated_swap_two_no_rated() {
    ...
    assert_eq!(
        view!(pool
            .mft_balance_of(":0".to_string(), to_va(root.account_id.clone())))
        .unwrap_json::<U128>()
        .0,
        200000 * ONE_LPT
    );
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            token_account_ids: vec![near(), stnear()],
            amounts: vec![
                U128(100001 * ONE_NEAR),
                U128(99999 * ONE_STNEAR + 2500000498725063547331)
            ],
            total_fee: 25,
...
fn sim_rated_swap_rate_one_with_fee() {
    ...
    let rated_infos = view!(pool.list_rated_tokens())
        .unwrap_json::<HashMap<String, RatedTokenInfo>>();
...
fn sim_rated_swap_rate_one_no_fee() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100001 * ONE_NEAR),
                U128(99999 * ONE_STNEAR + 499975001049956)
            ],
            total_fee: 0,
            shares_total_supply: U128(200000 * ONE_LPT),
        }
...
fn sim_rated_swap_three_no_rated() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            token_account_ids: vec![near(), stnear(), linear()],
            amounts: vec![
                U128(100002 * ONE_NEAR),
                U128(99999 * ONE_STNEAR + 2500000110832101864303),
                U128(99999 * ONE_STNEAR + 2500000221661989000175)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                300000 * ONE_LPT
                    + 499999996666583725184
                    + 499999993277742563392
            ),
        }
    );
}
...
fn sim_rated_swap() {
    ...
    assert_eq!(
        "STNEAR".to_string(),
        rated_infos.get(&stnear_contract.account_id()).unwrap().rate_type
    );
    assert_eq!(
        10u128.pow(24),
        rated_infos.get(&stnear_contract.account_id()).unwrap().rate_price.0
    );
    assert_eq!(
        0,
        rated_infos
            .get(&stnear_contract.account_id())
            .unwrap()
            .last_update_ts
            .0
    );
    assert_eq!(
        false,
        rated_infos.get(&stnear_contract.account_id()).unwrap().is_valid
    );
    ...
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come)
        .contains("Rated token stnear already exist"));
    ...
    assert_eq!(
        "LINEAR".to_string(),
        rated_infos.get(&linear_contract.account_id()).unwrap().rate_type
    );
    assert_eq!(
        10u128.pow(24),
        rated_infos.get(&linear_contract.account_id()).unwrap().rate_price.0
    );
    assert_eq!(
        0,
        rated_infos
            .get(&linear_contract.account_id())
            .unwrap()
            .last_update_ts
            .0
    );
    assert_eq!(
        false,
        rated_infos.get(&linear_contract.account_id()).unwrap().is_valid
    );
...
fn sim_rated_swap_register_unregister() {
    ...
    out_come.assert_success();
    assert!(get_logs(&out_come)
        .contains(&"Rated token linear removed.".to_string()));
    let rated_infos = view!(pool.list_rated_tokens())
        .unwrap_json::<HashMap<String, RatedTokenInfo>>();
    assert_eq!(1, rated_infos.len());
    ...
    out_come.assert_success();
    assert!(get_logs(&out_come)
        .contains(&"Rated token linear not exist in rate list.".to_string()));
    let rated_infos = view!(pool.list_rated_tokens())
        .unwrap_json::<HashMap<String, RatedTokenInfo>>();
    assert_eq!(1, rated_infos.len());
    ...
    out_come.assert_success();
    assert!(get_logs(&out_come)
        .contains(&"Rated token stnear removed.".to_string()));
    let rated_infos = view!(pool.list_rated_tokens())
        .unwrap_json::<HashMap<String, RatedTokenInfo>>();
...
fn sim_rated_swap_lp() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            token_account_ids: vec![near(), stnear(), linear()],
            amounts: vec![
                U128(100500 * ONE_NEAR),
                U128(100600 * ONE_STNEAR),
                U128(100800 * ONE_LINEAR)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                301200 * ONE_LPT
                    + 699699997426210330024139704
                    + 47999999735823255901269
            ),
        }
    );
    assert_eq!(
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        699699997426210330024139704
    );
    assert!(pool_share_price(&pool, 0) > last_share_price);
    let last_share_price = pool_share_price(&pool, 0);
    assert_eq!(
        mft_total_supply(&pool, ":0"),
        last_lpt_supply + 699699997426210330024139704 + 47999999735823255901269
    );
    let last_lpt_supply =
        last_lpt_supply + 699699997426210330024139704 + 47999999735823255901269;
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            token_account_ids: vec![near(), stnear(), linear()],
            amounts: vec![
                U128(100499 * ONE_NEAR),
                U128(100100 * ONE_STNEAR),
                U128(100799 * ONE_LINEAR)
            ],
            total_fee: 25,
            shares_total_supply: U128(
                last_lpt_supply - 502598491280079770547579295
                    + 95823884420348155736299
            ),
        }
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user1.account_id()),
        1200 * ONE_LPT - 502598491280079770547579295
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        699699997426210330024139704
    ...
    let last_share_price = pool_share_price(&pool, 0);
    let last_lpt_supply =
        last_lpt_supply - 502598491280079770547579295 + 95823884420348155736299;
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user1.account_id()),
        1100 * ONE_LPT - 502598491280079770547579295
    );
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        799699997426210330024139704
    );
    assert_eq!(pool_share_price(&pool, 0), last_share_price);
    ...
    assert_eq!(
        mft_balance_of(&pool, ":0", &user2.account_id()),
        499699997426210330024139704 - 498596260777261245962554635
    );
    // last_lpt_supply - removed lpt + admin_fee_to_lpt
    let last_lpt_supply =
        last_lpt_supply - 498596260777261245962554635 + 95600058313712936588149;
    assert_eq!(mft_total_supply(&pool, ":0"), last_lpt_supply);
    assert!(pool_share_price(&pool, 0) > last_share_price);
    let last_share_price = pool_share_price(&pool, 0);
    ...
    mint_and_deposit_token(
        &user3,
        &tokens[0],
        &pool,
        100_000_000_000 * ONE_NEAR,
    );
    mint_and_deposit_rated_token(
        &user3,
        &token_rated_contracts[0],
        &pool,
        100_000_000_000 * ONE_STNEAR,
    );
    mint_and_deposit_rated_token(
        &user3,
        &token_rated_contracts[1],
        &pool,
        100_000_000_000 * ONE_LINEAR,
    );
    let out_come = call!(
        user3,
        pool.add_stable_liquidity(
            0,
            vec![
                U128(100_000_000_000 * ONE_NEAR),
                U128(100_000_000_000 * ONE_STNEAR),
                U128(100_000_000_000 * ONE_LINEAR)
            ],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
    ...
    // minted_user_lpt
    assert_eq!(
        mft_balance_of(&pool, ":0", &user3.account_id()),
        299997852137498188726148212849465927
    );
    ...
    let last_lpt_supply = last_lpt_supply
        + 299997852137498188726148212849465927
        + 143329282015797902428444724880;
    assert_eq!(mft_total_supply(&pool, ":0"), last_lpt_supply);
    let last_share_price = pool_share_price(&pool, 0);
    println!("share_price: {}", last_share_price);
}
...
fn sim_rated_swap_max_liquidity() {
    ...
    assert_eq!(
        mft_total_supply(&pool, ":0"),
        300000300000000000000000000000000000
    );
    let last_share_price = pool_share_price(&pool, 0);
    println!("share_price: {}", last_share_price);
}
...
fn sim_rated_swap_lp_storage() {
    ...
    let user = root.create_user("user".to_string(), to_yocto("100"));
    call!(user, near_token.mint(user.valid_account_id(), U128(500 * ONE_NEAR)))
        .assert_success();

    call!(user, pool.storage_deposit(None, None), deposit = to_yocto("0.0025"))
        .assert_success();
    ...
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![U128(500 * ONE_NEAR), U128(0), U128(0)],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
    );
...
fn sim_rated_swap_liquidity_two_with_nearx() {
    let (root, owner, pool, tokens, token_rated_contracts) =
        setup_rated_pool(vec![near()], vec![nearx()], vec![24, 24], 25, 10000);
    ...
    call!(root, nearx_contract.set_price(U128(2 * 10u128.pow(24))))
        .assert_success();

    call!(
        owner,
        pool.update_token_rate(nearx_contract.valid_account_id()),
        deposit = 1
    )
    .assert_success();
    ...
    let out_come = call!(
        ...
        pool.add_stable_liquidity(
            0,
            vec![U128(100000 * ONE_NEAR), U128(50000 * ONE_NEARX)],
            U128(1)
        ),
        deposit = to_yocto("0.0007")
    );
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(200000 * ONE_LPT),
            vec![U128(1 * ONE_NEAR), U128(1 * ONE_NEARX)]
        ),
        deposit = 1
    );
    out_come.assert_success();
...
fn sim_rated_swap_two_no_rated_with_nearx() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100001 * ONE_NEAR),
                U128(99999 * ONE_NEARX + 2500000498725063547331)
            ],
            total_fee: 25,
            shares_total_supply: U128(200000 * ONE_LPT + 499999994999720058346),
        }
...
fn sim_rated_swap_rate_one_no_fee_with_nearx() {
    ...
    assert_eq!(
        ...
        PoolInfo {
            ...
            amounts: vec![
                U128(100001 * ONE_NEAR),
                U128(99999 * ONE_NEARX + 499975001049956)
            ],
            total_fee: 0,
            shares_total_supply: U128(200000 * ONE_LPT),
        }
