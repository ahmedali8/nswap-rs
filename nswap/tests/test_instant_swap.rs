...
fn instant_swap_scenario_01() {
    ...
    assert!(get_error_status(&out_come)
        .contains("E28: Illegal msg in ft_transfer_call"));
    ...
    println!(
        "Case 0103: non-registered user swap but not registered in token2"
    );
    let action =
        pack_action(0, &token1.account_id(), &token2.account_id(), None, 1);
    let out_come = direct_swap(&new_user, &token1, vec![action], to_yocto("1"));
    out_come.assert_success();
    // println!("{:#?}", out_come.promise_results());
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come).contains(
        "Smart contract panicked: The account new_user is not registered"
    ));
...
fn instant_swap_scenario_02() {
    let (root, owner, pool, token1, token2, token3) =
    ...
    // println!("total logs: {:#?}", get_logs(&out_come));
    assert!(get_logs(&out_come)[2].contains(
        "Account new_user has not enough storage. Depositing to owner."
    ));
    assert_eq!(
        get_storage_balance(&pool, new_user.valid_account_id())
            .unwrap()
    ...
    call!(new_user, pool.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();
    call!(
        new_user,
...
fn instant_swap_scenario_03() {
    ...
    .assert_success();
    call!(new_user, pool.storage_withdraw(None), deposit = 1).assert_success();

    println!("Case 0301: two actions with one output token");
    let action1 =
        pack_action(0, &token1.account_id(), &token2.account_id(), None, 1);
    let action2 =
        pack_action(1, &token2.account_id(), &token3.account_id(), None, 1);
    let out_come =
        direct_swap(&new_user, &token1, vec![action1, action2], to_yocto("1"));
    out_come.assert_success();
    // println!("{:#?}", out_come.promise_results());
    assert_eq!(get_error_count(&out_come), 0);
    ...
    call!(new_user, token2.storage_unregister(Some(true)), deposit = 1)
        .assert_success();
    ...
    .assert_success();
    call!(new_user, token3.storage_unregister(Some(true)), deposit = 1)
        .assert_success();
...
fn instant_swap_scenario_04() {
    ...
    let (root, owner, pool, tokens) = setup_stable_pool_with_liquidity(
        vec![dai(), usdt(), usdc()],
    ...
    call!(user, token_in.mint(user.valid_account_id(), U128(10 * ONE_DAI)))
        .assert_success();
    ...
    assert_eq!(get_error_count(&out_come), 1);
    assert!(get_error_status(&out_come).contains(
        "Smart contract panicked: The account user is not registered"
    ));
    ...
    call!(user, token_out.storage_deposit(None, None), deposit = to_yocto("1"))
        .assert_success();
    let action = pack_action(
        0,
        &tokens[0].account_id(),
