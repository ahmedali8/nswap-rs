...
fn storage_scenario_01() {
    ...
    let out_come = call!(new_user, pool.storage_withdraw(None), deposit = 1);
    ...
    let prev_sb =
        get_storage_balance(&pool, new_user.valid_account_id()).unwrap();
    let orig_user_balance = new_user.account().unwrap().amount;
    // println!("{:#?}", prev_sb);
...
fn storage_scenario_02() {
    ...
    println!(
        "Storage Case 0201: appending balanced liquidity need deposit storage"
    );
    call!(
        new_user,
        pool.add_stable_liquidity(
            0,
            vec![U128(10 * ONE_DAI), U128(10 * ONE_USDT), U128(10 * ONE_USDC)],
            U128(1)
        ),
    ...
    println!(
        "Storage Case 0202: appending imba liquidity need deposit storage"
    );
    let out_come = call!(
        new_user,
        pool.add_stable_liquidity(
            0,
            vec![U128(5 * ONE_DAI), U128(10 * ONE_USDT), U128(15 * ONE_USDC)],
            U128(1)
        ),
        deposit = to_yocto("0.00074")
    );
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity(
            0,
            U128(10 * ONE_LPT),
            vec![U128(3 * ONE_DAI), U128(3 * ONE_USDT), U128(3 * ONE_USDC)]
        ),
        deposit = 1
    );
    out_come.assert_success();
    ...
    let out_come = call!(
        ...
        pool.remove_liquidity_by_tokens(
            0,
            vec![U128(10 * ONE_DAI), U128(1 * ONE_USDT), U128(1 * ONE_USDC)],
            U128(13 * ONE_LPT)
        ),
        deposit = 1
    );
    ...
    let out_come = call!(user2, pool.storage_withdraw(None), deposit = 1);
    out_come.assert_success();
    ...
    println!(
        "Storage Case 0206: transfer lp would fail if receiver not registered"
    );
